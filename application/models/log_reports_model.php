...
class Log_Reports_Model extends CI_Model {
    ...
    public function alert_log_reports($where='', $sort='', $limit='') {
        ...
        if($sort && $limit){
            ...
            return $query = $this->db->query("select msisdn,circle,operator,processed_msg,original_msg,response_sms,to_char(log_time, 'DD-MM-YY HH24:MI:SS') as log_time from playwin_sc_log $where $sort");
        }else{
            $count = $this->db->query("select count(1) from playwin_sc_log $where")->row_array();
            return $count['count'];
        }
